...
public class OrnamentBlock extends FacingBlock {
    ...
    protected ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!player.shouldCancelInteraction() && !player.getMainHandStack().isOf(this.asItem())) {
            ...
            return ((AbstractBlockInvoker) blockBehindOrnament.getBlock()).wathe$invokeOnUseWithItem(stack, blockBehindOrnament, world, behindBlockPos, player, hand, hit.withBlockPos(behindBlockPos));
        }
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!player.shouldCancelInteraction() && !player.getMainHandStack().isOf(this.asItem())) {
            ...
            BlockPos behindBlockPos = pos.subtract(new Vec3i(dir.getOffsetX(), dir.getOffsetY(), dir.getOffsetZ()));
            BlockState blockBehindOrnament = world.getBlockState(behindBlockPos);
            return ((AbstractBlockInvoker) blockBehindOrnament.getBlock()).wathe$invokeOnUse(blockBehindOrnament, world, behindBlockPos, player, hit.withBlockPos(behindBlockPos));
        }
